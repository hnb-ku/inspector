    ...
    withPluginApi("0.8", api => {
      ...
      const tooltippyboi = event => {
        ...
        *
        ...
        if (getMatchedCSSRules(target).length) {
          styles = getMatchedCSSRules(target).map(rule => rule.cssText);
          selectors = getMatchedCSSRules(target).map(rule => rule.selectorText);
          //
        } else if (getMatchedCSSRules(target.parentNode).length) {
          styles = getMatchedCSSRules(target.parentNode).map(
            rule => rule.cssText
          );

          selectors = getMatchedCSSRules(target.parentNode).map(
        ...
        console.log(
          getMatchedCSSRules(target) || getMatchedCSSRules(target.parentNode)
        );

        console.log(typeof styles);

        const cleanSelectors = selectors
          .filter((item, index) => selectors.indexOf(item) === index)
          .join("\n");
...
*/
        ...
        if (css.length) {
          const content =
            `<h3>Selectors</h3>` +
            `<pre><code contentEditable="true" data-gramm_editor="false">${cleanSelectors}</code></pre>` +
            `<h3>Styles</h3>` +
            `<pre><code contentEditable="true" data-gramm_editor="false" class="lang-css hljs">${css}</code></pre>`;
...
import { CSSUtilities } from "discourse/lib/cssutil";
