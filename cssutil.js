...
(function() {
  ...
  var THIS = this; //set the asynchronous ready flag to false

  this._allready = false; //set the supported flag to false by default

  this.supported = false; //do some feature detection

  this.supported = true; //-- start local variable declarations --//
  ...
  var busyinit = false,
    ...
    api = NULL_VALUE,
    qsa = NULL_VALUE,
    apidefined = BOOLEAN_FALSE; //-- public method for modifying privileged variables --//
  ...
  this.define = function(varname, value1, value2) {
    ...
    switch (varname) {
      ...
      case "mode":
        ...
        mode = value1; //if mode is now "browser" check that "watch" is not null
      ...
      case "api":
        ...
        apidefined = BOOLEAN_TRUE; //if we have a function definition, pre-test it to make sure
  ...
  this.init = function(callback) {
    ...
    busyinit = BOOLEAN_TRUE; //clear the global ready flag

    this._allready = BOOLEAN_FALSE; //if querySelectorAll is supported, and api has not been set to true, set it to false
    ...
    api =
      typeof page.querySelectorAll != TYPE_UNDEFINED && api !== BOOLEAN_TRUE
        ? BOOLEAN_FALSE
        : BOOLEAN_TRUE; //if base hasn't been defined, do so now
    ...
    this._isXML =
      ...
      (!OPERA &&
        ...
        page.xmlVersion != NULL_VALUE); //get the current page view media

    this._viewmedia = getViewMedia(); //nullify and delete the rules data and debug stylesheets array
    ...
    this._cssRules = undefined; // delete this._cssRules;

    this._stylesheets = undefined; // delete this._stylesheets;
    //define the local callback

    function localcallback() {
      //clear the busy init flag
      busyinit = BOOLEAN_FALSE; //set the global ready flag

      CSSUtilities._allready = BOOLEAN_TRUE; //if callback is a function, save the reference and call it
    ...
    THIS._cssRules = [];
    THIS._stylesheets = []; //start a new ssidcounter for assigning stylesheet IDs

    THIS.ssidcounter = 0; //[re]create the core rules data array according to mode
  ...
  this.getCSSStyleSheets = function() {
    ...
    var args = processArguments(arguments, []); //process the onfinished argument,
    //which sets it to null if undefined or not a function, for ease of reference

    args.onfinished = processOnfinishedArgument(args.onfinished); //if the script hasn't been initialized, do so now

    lateinit(); //create an internal wrapper for the rest of this function
  ...
  this.getCSSRules = function() {
    ...
    args.element = processElementArgument(args.element, "getCSSRules"); //process the media argument
    //this sets default if undefined, empty or null
    //then splits it into a trimmmed array

    args.media = processMediaArgument(args.media); //process the accept argument
    ...
    args.accept = processAcceptArgument(args.accept); //if altstates is null, make it false
  ...
  this.getCSSStyleSheetRules = function() {
    //process the arguments to create an indexed object
    var args = processArguments(arguments, ["media", "accept", "ssid"]); //process the media argument
    //this sets default if undefined, empty or null
    //then splits it into a trimmmed array

    args.media = processMediaArgument(args.media); //process the accept argument

    args.accept = processAcceptArgument(args.accept); //if accept contains "properties" but not "css"
    ...
    lateinit(); //if ssid is specified but invalid (ie. there's no stylesheet with that ssid), throw an error
    ...
    return dispatchReturn(args.onfinished, function() {
      ...
      var rules = []; //now iterate through the main rules array to build the data

      for (var i = 0; i < THIS._cssRules.length; i++) {
        //create the object for this rule
        var cssrule = {}; //if the media of this rule matches the input criteria
  ...
  this.getCSSProperties = function() {
    //process the arguments to create an indexed object
    var args = processArguments(arguments, ["element", "media"]); //process the element argument
    //this creates an element reference from an ID
    //and/or throws an error if the element us undefined, null, or not an element

    args.element = processElementArgument(args.element, "getCSSProperties"); //process the media argument
    //this sets default if undefined, empty or null
    //then splits it into a trimmmed array

    args.media = processMediaArgument(args.media); //process the onfinished argument,
    //which sets it to null if undefined or not a function, for ease of reference
    ...
    return dispatchReturn(args.onfinished, function() {
      ...
      var properties = {}; //get all the rules that apply to this element and media
  ...
  this.getCSSSelectors = function() {
    //process the arguments to create an indexed object
    var args = processArguments(arguments, ["element", "media", "directonly"]); //process the element argument
    //this creates an element reference from an ID
    //and/or throws an error if the element us undefined, null, or not an element

    args.element = processElementArgument(args.element, "getCSSSelectors"); //process the media argument
    //this sets default if undefined, empty or null
    //then splits it into a trimmmed array

    args.media = processMediaArgument(args.media); //if the directonly flag is null, default to true
    ...
    return dispatchReturn(args.onfinished, function() {
      ...
      var allselectors = []; //now get all the rules that apply to this element and media
      ...
      var selectors = []; //iterate through the allselectors array
  ...
  this.getCSSSelectorSpecificity = function() {
    //process the arguments to create an indexed object
    var args = processArguments(arguments, ["selector", "element"]); //if selector is undefined, null, empty, or not a string, throw the no selector error
    ...
    return dispatchReturn(args.onfinished, function() {
      //trim the selector
      args.selector = trim(args.selector); //if the element argument is in use, work out whether this selector applies to it
  ...
  function createAuthorRulesData(oncomplete) {
    ...
    THIS._cssdata = []; //if this is IE and the page contains any style element
    ...
    function continuation() {
      ...
      var stylenodes = getStyleSheetNodes(); //now iterate through the collection of style nodes
      ...
      function stage1_checkStyleSheetForDataWrapper(cdcount) {
        ...
        stage2_checkStyleSheetForData(
          THIS._cssdata[cdcount], //and when that completes
          function(cssdata) {
            //save back the modified object
            THIS._cssdata[cdcount] = cssdata; //if we have more to process, recur
      ...
      function stage3_checkStyleSheetForImportsWrapper() {
        ...
        function stage4_checkStyleSheetForImports(cdindex) {
          ...
          if (THIS._cssdata[cdindex].tested == BOOLEAN_TRUE) {
            ...
            THIS._cssdata[cdindex].tested = BOOLEAN_TRUE; //assign its ssid; we waited until now to do this
            //so that we can assign them in order of occurence

            THIS._cssdata[cdindex].ssid = THIS.ssidcounter++; //if we have any imports
            ...
            if (importsheets.length > 0) {
              ...
              function stage5_checkImportForDataWrapper(impindex) {
                ...
                stage6_checkImportForData(
                  cdindex,
                  impindex,
                  importsheets[impindex], //and when that completes
                  function(cssdata, cdnumber, impnumber) {
                    ...
                    THIS._cssdata.splice(cdindex + impnumber, 0, cssdata); //if there are any more imports to check, recur on the next one
      ...
      function stage6_checkImportForData(
        ...
        cssdata.media = importobj.media; //get the context from the nearest ancestral non-import
        ...
        context = parent.media; //if the import itself has its own media specified, save that
        ...
        cssdata.xmedia = importobj.hasownmedia ? importobj.media : MEDIA_ALL; //then contextualize the import's media

        cssdata.media = contextualizeMediaTypes(context, cssdata.media); //local finalizing function
      ...
      function stage7_processStyleSheetsData() {
        ...
        for (var i = 0; i < THIS._cssdata.length; i++) {
          ...
          var rulecount = parseStyleSheetCSSText(THIS._cssdata[i]); //record an entry to the debug stylesheets array
        ...
        delete THIS._cssdata; //do a final sort of the _stylesheets array, putting it in ssid order
        ...
        delete THIS.ssidcounter; //if the watch setting is strictly true, watch for changes in disabled state
  ...
  function getRawStyleData(oncomplete) {
    ...
    ajaxload(
      async,
      base, //if the load succeeds
      function(code) {
        //re-get the "style" elements collection
        var stylenodes = getTheseElements("style"); //save a copy of the html and split it by opening style tags

        var styleblocks = code.split(/<([^:]+:)?style[^>]*>/i); //delete the first member, which will be
        //everything up to the first style element

        styleblocks.splice(0, 1); //now iterate and split each member by closing style tags
        ...
        styleblocks = undefined; // delete styleblocks;
        //call the callback
  ...
  function getStyleSheetNodes() {
    //create the array
    var stylenodes = []; //begin by looking for xml-stylesheet processing instructions
    ...
    var allnodes = getTheseElements("*"); //then iterate through that to look for stylesheet nodes
  ...
  function processStyleNode(node, owner) {
    ...
    cssdata.media = cssdata.media.split(/,\s*/).join("," + " "); //and copy it to the xmedia property

    cssdata.xmedia = cssdata.media; //add this data object to the array
  ...
  function requestStyleSheet(href, oncomplete) {
    ...
    ajaxload(
      async,
      href, //if it succeeds, save the text content to the data object
      ...
      function(statustext) {
        ...
        requestdata.message = statustext; //call oncomplete with the request data
  ...
  function extractImportStatements(styletext) {
    ...
    var importsheets = []; //define regexes for matching import statements
    //which may include media definitions and queries

    var importreg = /@import\s*(?:url\s*\(\s*[\'\"]?|[\'\"])([^\'\"\)]+)(?:[\'\"]|[\'\"]?\s*\))([^;]*)(;|$)/gi; //look for @import statements in the text

    var matches = styletext.match(importreg); //if we have matches

    if (matches) {
      for (var i = 0; i < matches.length; i++) {
        //extract and qualify the href
        var href = qualifyHREF(matches[i].replace(importreg, "$1"), base); //extract the media, from which we may have to trim
        ...
        var hasownmedia = BOOLEAN_TRUE; //then if it didn't in fact one, set this flag to false,
  ...
  function strainStyleSheet(styletext, importsheets) {
    ...
    styletext = styletext.replace(/(\/\*([^*]|(\*+([^*/])))*\*+\/)/gm, ""); //in HTML mode, delete anything inside and including CDATA markers
    ...
    styletext = styletext.replace(/[\t]+/g, " ").replace(/[ ][ ]/g, " "); //line-breaks

    styletext = styletext.replace(/[\r\n]/g, ""); //@charset and @namespace statements

    styletext = styletext.replace(/@(charset|namespace)[^;]+;/gim, ""); //if the importsheets flag is defined and true, remove @import statements
    ...
    //so any which come after a block of normal CSS don't apply and get deleted
      ...
      var cssdata = styletext
        ...
        .split("{SPLIT}"); //then clear the original text variable and iterate through the fragments
    ...
    styletext = styletext.replace(/@(font\-face|page)[^\}]+\}/gim, ""); //remove any remaining HTML comments or CDATA markers
  ...
  function parseStyleSheetCSSText(cssdata) {
    ...
    var rulecount = 0; //we identify changes in media by an odd number of members in a group of text lines
    //beginning with the default media values from the stylesheet

    var currentmedia = cssdata.media,
      currentxmedia = cssdata.xmedia; //we identify changes in owner the same way

    var currentowner = cssdata.owner; //iterate through the text lines
    ...
    for (var i = 0; i < cssdata.text.length; i++) {
      ...
      if (cssdata.text[i].length == 2) {
        ...
        currentowner = "@media"; //copy to currentxmedia to save the original value

        currentxmedia = currentmedia; //then adjust the media type if necessary to take account of the context
        ...
        currentmedia = contextualizeMediaTypes(context, currentmedia); //add this rule to the cssRules array
  ...
  function createBrowserRulesData(oncomplete) {
    ...
    var stylesheets = arrayifize(page.styleSheets); //iterate through the collection and parse each stylesheet

    for (var i = 0; i < stylesheets.length; i++) {
      //assign an index to this stylesheet
      stylesheets[i].__ssid = THIS.ssidcounter++; //if the stylesheets has an ownerNode property then this is a standard implementation
      ...
      if (typeof stylesheets[i].ownerNode != TYPE_UNDEFINED) {
        ...
        if (
          ...
          try {
            parseStyleSheet(
              stylesheets[i],
              stylesheets[i].ownerNode.nodeName.toLowerCase(), //the trim probably isn't necessary, but better safe than sorry!
      ...
      //but at least it has the excuse of that being the canonical form
        ...
        parseIEStyleSheet(
          stylesheets[i],
          stylesheets[i].owningElement.nodeName.toLowerCase(), //the trim probably isn't necessary, but better safe than sorry!
    ...
    THIS.ssidcounter = undefined; // delete THIS.ssidcounter;
  ...
  function parseStyleSheet(sheet, owner, media) {
    ...
    if (sheet.href && sheet.href != base) {
      ...
      if (arrayContains(THIS._stylesheets, testhref, "href") != NULL_VALUE) {
        ...
        sheet.disabled = BOOLEAN_TRUE; //and we're done here
    ...
    try {
      ...
      var isdisabled = sheet.disabled; //in opera, the stylesheet returns as disabled if it applies to media
      //other than the one currently in force, such as "print" when the view is "screen"
      //so what we'll do is temporarily switch the media to whatever the current view is
      //then read its disabled property again, to find out if it's
      //really disabled or just applicable to a non-view media
  ...
  function parseCSSRules(sheet, rules, owner, media) {
    ...
    for (i = 0; i < rules.length; i++) {
      ...
      rule = rules.item(i); //if it's an @import
      ...
      if (rule.type == 3) {
        ...
        media = media.toLowerCase(); //then adjust the media type if necessary to take account of the context
        ...
        while (parent) {
          ...
          media = contextualizeMediaTypes(context, media); //this condition is to catch a difference in parent stylesheet reference
    ...
    for (var i = 0; i < rules.length; i++) {
      //save a shortcut reference
      var rule = rules.item(i); //if this is a regular style rule
      ...
      if (rule.type == 1) {
        ...
        var styletext = rule.style.cssText.replace(/[\r\n]/g, " "); //split the cssText by semi-colons

        styletext = styletext.split(";"); //then iterate through the results and splt each one by its first colon
        ...
        styletext = styletext.join(";"); //add this rule object to the array
    ...
    for (i = 0; i < rules.length; i++) {
      //save a shortcut reference
      rule = rules.item(i); //if it's an @media
  ...
  function parseIEStyleSheet(sheet, owner, media, href) {
    ...
    var importsheets = []; //is this stylesheet disabled

    var isdisabled = sheet.disabled; //if the stylesheet is not disabled
    ...
    if (!isdisabled) {
      ...
      for (var i = 0; i < sheet.imports.length; i++) {
        ...
        var ihref = qualifyHREF(sheet.imports[i].href, base); //add the impored stylesheet to the imports array
    ...
    try {
      ...
      if (!isdisabled) {
        //first copy the text
        var styletext = sheet.cssText; //remove any @import, @charset and @namespace statements
        ...
        styletext = styletext.replace(/@(font\-face|page)[^\}]+\}/gim, ""); //do token replacements for generated content
        //to avoid confusion in case it contains any brace characters
        ...
        styletext = trim(styletext).split("}"); //iterate through the resulting lines
        //trim and split each one again, by the opening brace character
        ...
        var rules = []; //we identify changes in media by an odd number of members in a group of text lines
        ...
        var currentowner = owner; //iterate through the text lines
        ...
        for (var i = 0; i < styletext.length; i++) {
          ...
          //which we can edit to get the actual media value
            ...
            currentowner = "@media"; //adjust the media type if necessary to take account of the context
  ...
  function parseIECSSRules(rules) {
    ...
    var rulecount = 0; //iterate through the rules collection
  ...
  function getCSSRules(
    ...
    accept = processAcceptArgument(accept); //if rules is undefined or null, define a new array
    ...
    if (typeof ancestors == TYPE_UNDEFINED || ancestors == NULL_VALUE) {
      ancestors = [element]; //if the element has a style attribute, and the "attributes" setting is true,
      ...
      if (nodes.length > 0) {
        for (var j = 0; j < nodes.length; j++) {
          ...
          if (nodes[j] == element) {
            ...
            rules.push(ruleobj); //we'll only find it once!
    ...
    if (element.parentNode && element.parentNode.nodeType == 1) {
      ...
      for (var i = 0; i < rules.length; i++) {
        ...
        var selectors = rules[i].selector.split(","); //now we need to pass each selector to the getSelectorSpecificity method
        ...
        for (var i = 0; i < rules.length; i++) {
          for (
            ...
            if (typeof accept[props[j]] == TYPE_UNDEFINED) {
              rules[i][props[j]] = undefined; // rules[i][props[j]] = NULL_VALUE;
  ...
  function addSortedProperties(rules, dosort) {
          ...
          for (var j = 0; j < i; j++) {
            ...
            for (var jp in rules[j].allproperties) {
              ...
              for (var ip in rules[i].allproperties) {
                ...
                if (jp == ip) {
                  rules[j].allproperties[jp].status = STATUS_CANCELLED; //there will only be one
      ...
      for (var i = 0; i < rules.length; i++) {
        ...
        for (var j in rules[i].allproperties) {
          ...
          if (IMPORTANT_RULE.test(rules[i].allproperties[j].value)) {
            ...
            var precedence = i; //then run through every subsequent object
            ...
            for (var x = i; x < rules.length; x++) {
              ...
              if (x == precedence) {
                rules[x].allproperties[j].status = STATUS_ACTIVE; //and any earlier active instances of that property to which we did the same thing
              ...
              //update its status to "cancelled"
                ...
                if (
                  ...
                  rules[x].allproperties[j].status = STATUS_CANCELLED; //there will only be one
    ...
    for (var i = 0; i < rules.length; i++) {
      ...
      rules[i].allproperties = undefined; // delete rules[i].allproperties;
  ...
  function getSelectorSpecificity(selector) {
    ...
    var scores = [0, 0, 0, 0]; //if the selector is an empty string, this indicates a style attribute
    ...
    var editedselector = selector.replace(REGEX_ATTR_SELECTORS, ""); //look for ID selectors, which have the highest specificity category
    ...
    var matches = editedselector.match(REGEX_ID_SELECTORS); //add the number of matches (if any) to the id score
    ...
    var matches = editedselector.match(REGEX_CLASS_SELECTORS); //add the number of matches (if any) to the class score
    ...
    matches = selector.match(REGEX_ATTR_SELECTORS); //add the number of matches (if any) to the class score
    ...
    var matches = editedselector.match(REGEX_PSEUDO_CLASSES_EXCEPT_NOT); //add the number of matches (if any) to the class score
    ...
    var matches = typeonlyselector.match(/([_a-z0-9-:\\]+)/gi); //add the number of matches (if any) to the type score
    ...
    var matches = editedselector.match(REGEX_PSEUDO_ELEMENTS); //add the number of matches (if any) to the type score
  ...
  function lowercaseIEPropertyNames(styletext) {
    ...
    styletext = styletext.split(";"); //iterate through the resulting properties
    ...
    for (var j = 0; j < styletext.length; j++) {
      ...
      matches[1] = trim(matches[1]); //then join them back together with consistent spacing
  ...
  function parseSelectorText(selectorstring) {
    ...
    var selectors = []; //if the selectorstring is empty just return the empty array

    if (selectorstring == "") {
      return selectors;
    } //split the selector by commas

    selectorstring = selectorstring.split(","); //iterate through and add each trimmed selector to the array
  ...
  function parsePropertyText(styletext, properties, inherited) {
    ...
    styletext = styletext.replace(/[\r\n]/gm, ""); //split the css text by semi-colon to get individual property definitions

    styletext = styletext.split(";"); //now iterate through that array and create an object member for each one
    ...
    for (var i = 0; i < styletext.length; i++) {
      ...
      var key = trim(matches[0]); //if the rule is not inherited, or the property is inheritable
  ...
  function contextualizeMediaTypes(context, types) {
    ...
    var contextTypes = lexiconize(context, ","); //we're not currently testing for media query context
    ...
    var mediaQuery = /([ \t]and.*$)/i,
      ...
      typesQuery = matches ? matches[0] : ""; //we have to remove any "only" token so that we can do comparisons
    ...
    types = types.replace(mediaQuery, "").replace(/only[ \t]+/i, ""); //if the contextTypes object contains "all" then any inner types are okay
  ...
  function mediaMatches(media, rule) {
    ...
    var mediaTypes = lexiconize(rule.media, ","); //begin by assuming no match

    var matches = BOOLEAN_FALSE; //then iterate through the input array of media types to look for at least one match
  ...
  function getViewMedia() {
    ...
    var viewmedia = MEDIA_SCREEN; //wrap all this in exception handling
    ...
    try {
      ...
      testmedianode.style.display = "inline"; //create the array of all CSS media types; the list is a delimited string which is split here
      //cos that worked out to less code than declaring an array of separate strings :)

      var mediaTypes = MEDIA_TYPES_LIST.split(","); //we see a failure condition in Safari 3 and Konqueror, where it
      ...
      mediaTypes.splice(0, 0, "fake"); //create a stylesheet and add a rule for a test node we're going to create
      ...
      var stylesheet =
        mode == MODE_AUTHOR
          ? THIS._stylesheets[i].stylenode == BOOLEAN_FALSE ||
            ...
            ? NULL_VALUE
            : SAFARI || KONQUEROR || WEBKIT
            ? THIS._stylesheets[i].stylenode //nb. although we know this condition is IE vs. others, an object test is better
          ...
          : THIS._stylesheets[i].stylesheet; //ignore null stylesheets, such as duplicates,
    ...
    if ((SAFARI || KONQUEROR || WEBKIT) && mode == MODE_BROWSER) {
      ...
      var controlidents = createStyleSheetIdents(),
        watcher = window.setInterval(function() {
          ...
          var currentidents = createStyleSheetIdents(); //if either of the ident arrays has changed
  ...
  function processArguments(argsary, keys) {
    ...
    keys.push("onfinished"); //create the empty indexed arguments object
    ...
    for (var args = {}, i = 0; i < keys.length; i++) {
      ...
      if (typeof argsary[i] == TYPE_FUNCTION) {
        ...
        args.onfinished = argsary[i]; //and we're done
  ...
  function processMediaArgument(argmedia) {
    ...
    for (var i = 0; i < argmedia.length; i++) {
      argmedia[i] = trim(argmedia[i]); //if the value is "current" then set it to the current view media
  ...
  function processAcceptArgument(argaccept) {
    ...
    //on the first instance before it's already been processed
      ...
      //trim the value
      argaccept = trim(argaccept); //if the value is "*" just leave it
  ...
  function lexiconize(str, delimiter) {
    ...
    var obj = {}; //split the string by its delimiter

    str = str.split(delimiter); //if the final member contains media queries, split them off too
  ...
  function qualifyHREF(href, context) {
    ...
    parts.splice(0, 2);
    loc.pathname = "/" + parts.join("/"); //build a base URI from the protocol plus host (which includes port if applicable)
    //nb. the "/"+"/" is for the same minimizing reason

    var uri = loc.protocol + "/" + "/" + loc.host; //if the input path is relative-from-here
    ...
    //so just add the input href to the base URI
      ...
      lastpath = lastpath[lastpath.length - 1]; //count the number of up-references

      var references = href.split("../").length - 1; //get the path parts and delete the last one (this page or directory)

      var parts = loc.pathname.split("/");
      parts = parts.splice(0, parts.length - 1); //for each of the up-references, delete the last part of the path
      ...
      path += "/"; //and add the last part of the path

      path += lastpath; //then add the path and input href to the base URI
      ...
      path += "/"; //then add the path and input href to the base URI
  ...
  function ajaxload(async, uri, oncomplete, onfail) {
    ...
    requestobject.open("GET", uri, async); //try to set a request header that defines a library user-agent string
    ...
    } catch (err) {} //private response function
    ...
    if (async == BOOLEAN_TRUE) {
      ...
      requestobject.onreadystatechange = function() {
        ...
        if (requestobject.readyState == 4) {
          ...
          ajaxresponse(requestobject); //***DEV SEMI-RANDOMIZED LATENCY
          //}, (33 * Math.random()));
        }
      ...
      var success; //make the request inside the try construct
